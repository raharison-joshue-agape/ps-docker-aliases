...
dRenameContainer() {
    ...
    [ -z "$NewName" ] && read -r "NewName?Enter the new container name: "
...
dUpdateContainer() {
    ...
    [ -z "$Options" ] && read -r "Options?Enter update options (e.g. --memory 512m): "
...
dCommitContainer() {
    ...
    [ -z "$ImageName" ] && read -r "ImageName?Enter the new image name (e.g. myapp:latest): "
...
dCpContainer() {
    ...
    [ -z "$Source" ] && read -r "Source?Enter source path (host or container): "
    [ -z "$Destination" ] && read -r "Destination?Enter destination path (host or container): "
