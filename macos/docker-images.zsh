...
dBuildImage() {
    ...
    if [ -z "$AppName" ]; then
        read -r "AppName?Enter your app folder name: "
    fi
...
dTagImage() {
    ...
    [ -z "$SourceImage" ] && read -r "SourceImage?Enter source image (name:tag): "
    [ -z "$TargetImage" ] && read -r "TargetImage?Enter target image (name:tag): "
...
dLoadImage() {
    ...
    if [ -z "$InputFile" ]; then
        ...
        echo
        read -r "InputFile?Enter the path to the .tar file to load: "
    fi
